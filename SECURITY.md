...
| Secret scanning, non-provider patterns | enabled where the plan allows | **plan-gated off** | the credential class this repository is most likely to leak is not a provider token (an IXIT URL with embedded basic auth, the SMART test signing key), which is exactly what this setting catches — but it ships as part of GitHub Secret Protection, available to Team and Enterprise plans, and this user-owned Free-plan repository cannot enable it (the docs carry the availability note on the supported-patterns page; the API PATCH is silently ignored for the same reason). Re-adjudicated 2026-08-27, #115 |
| Secret scanning, validity checks | enabled where the plan allows | **plan-gated off** | the difference between "rotate this eventually" and "this credential is live right now" — but "validity and extended metadata checks are only available to users with GitHub Team or GitHub Enterprise who enable the feature as part of GitHub Secret Protection" (<https://docs.github.com/en/code-security/secret-scanning/introduction/supported-secret-scanning-patterns>). Re-adjudicated 2026-08-27, #115 |
